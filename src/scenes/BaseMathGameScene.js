...
export class BaseMathGameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const canY = h * 0.76;
    const trashY1 = h * 0.26;
    const trashY2 = h * 0.38;
    const xs = [w * 0.12, w * 0.30, w * 0.48, w * 0.66, w * 0.84];

    this.trashCans = xs.map((x, index) => new TrashCan(this, x, canY, answers[index]).setScale(1));
    this.trashItems = [
      new Trash(this, xs[0], trashY1, questions[0]).setScale(0.6),
      new Trash(this, xs[1], trashY2, questions[1]).setScale(0.6),
      new Trash(this, xs[2], trashY1, questions[2]).setScale(0.6),
      new Trash(this, xs[3], trashY2, questions[3]).setScale(0.6),
      new Trash(this, xs[4], trashY1, questions[4]).setScale(0.6),
    ];

    this.numGuessesPerAnswer = this.trashItems.map((trash) => ({ guessedAnswer: trash, numGuess: 0 }));
    this.numCorrect = 0;
    this.numWrong = 0;
  ...
  addCampBackground(w, h) {
    ...
    bg.setScale(scale).setAlpha(0.94);
    this.add.rectangle(w / 2, h / 2, w, h, 0x120d08, 0.18);
  }

  handleTrashDrop(trash) {
    if (this.gameIsFinished || !trash?.active) return;

    const matchingCan = this.trashCans.find((can) => can?.active && this.physics.overlap(trash, can));
    ...
    if (trash.answer === matchingCan.answer) {
      ...
      matchingCan.disableBody(true, true);
      trash.destroy();
      matchingCan.destroy();
      this.numCorrect += 1;
    ...
    this.numWrong += 1;
    const guessEntry = this.numGuessesPerAnswer.find((item) => item.guessedAnswer === trash);
  ...
  showFeedback(message, color) {
    ...
    this.feedbackText = this.add.text(40, 60, message, {
      fontFamily: "Arial",
      fontSize: "46px",
      color,
      stroke: "#3c2b16",
  ...
  async onFinish() {
    const w = this.scale.width;
    const panel = this.add.rectangle(w / 2, 250, Math.min(900, w * 0.9), 360, 0x1f1710, 0.72)
      .setStrokeStyle(6, 0xf1c75b)
      .setDepth(30);

    this.add.text(w / 2, 125, "Congratulations! You Finished!", TITLE_STYLE)
      .setOrigin(0.5)
      .setDepth(31);
